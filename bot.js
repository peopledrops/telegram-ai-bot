...
bot.onText(/\/learn\s+(.+)/i, async (msg, match) => {
    ...
    try {
        ...
        try {
            ...
            // Continue anyway - generate temp ID
...
⚠️ Catatan: Beberapa tasks perlu diselesaikan manual.
        ...
        await bot.sendMessage(chatId, 
            ...
            `💡 Coba lagi atau cek console bot untuk detail.`,
            { parse_mode: 'Markdown' }
        );
    }
});
...
💡 Complete social tasks for potential airdrops!
    ...
    try {
        ...
        // JANGAN kirim "Airdrop not found" untuk semua error
        // Hanya kirim untuk kasus tertentu
        if (error.message?.includes('not found')) {
            await bot.sendMessage(chatId, 
                `❌ Airdrop tidak ditemukan. Gunakan /airdrop list untuk melihat daftar.`,
                { parse_mode: 'Markdown' }
            );
        } else {
            await bot.sendMessage(chatId, `❌ Error: ${error.message}`);
...
bot.onText(/\/autoairdrop\s+(https?:\/\/\S+)/, async (msg, match) => {
    ...
    await bot.sendMessage(chatId, 
        `🚀 **Starting Full Auto Airdrop Completion!**\n\n` +
        `🔗 URL: ${url}\n` +
        `💼 Wallet: ${userWallet || 'Not set (use /wallet)'}\n\n` +
        `⏳ This will take 2-5 minutes. I'll update you on progress...`,
        { parse_mode: 'Markdown' }
    );
    ...
    try {
        ...
        if (result.success) {
             const taskResults = result.status.steps
        .filter(s => s.task)  // Only actual tasks, not system steps
        .map(s => {
            if (s.manual) {
...
- Check screenshots for auto-completed tasks
    ...
    await bot.sendMessage(chatId, report, { parse_mode: 'Markdown', disable_web_page_preview: true });
            ...
            await bot.sendMessage(chatId, 
                `❌ **Automation Failed**\n\n` +
                `Error: ${result.error}\n\n` +
                `Partial progress:\n${JSON.stringify(result.status.steps || [], null, 2)}`,
                { parse_mode: 'Markdown' }
            );
        }
    } catch (error) {
...
🗑️ Hapus profil: /clearprofile
    ...
    if (!validFields.includes(field)) {
        await bot.sendMessage(chatId, 
            `❌ Field tidak valid. Pilihan: ${validFields.join(', ')}`,
            { parse_mode: 'Markdown' }
        );
        return;
    }
    ...
    try {
        ...
        await bot.sendMessage(chatId, 
            ...
            `Gunakan /myprofile untuk lihat semua data.`,
            { parse_mode: 'Markdown' }
        );
...
bot.onText(/\/autofill\s+(https?:\/\/\S+)/, async (msg, match) => {
    ...
    if (!formAutoFiller || typeof formAutoFiller.autoSubmitForm !== 'function') {
        ...
        await bot.sendMessage(chatId, 
            '❌ **Auto-fill module error**\n\n' +
            'Module tidak ter-load dengan benar. Hubungi admin.',
            { parse_mode: 'Markdown' }
        );
        return;
    }
    ...
    if (!profile) {
        await bot.sendMessage(chatId, 
            '❌ **Profile belum diset!**\n\n' +
            'Gunakan /setprofile untuk menambahkan data sosial media Anda.',
            { parse_mode: 'Markdown' }
        );
        return;
    }
    ...
    await bot.sendMessage(chatId, 
        ...
        `⏳ Memproses... Ini butuh 30-60 detik.`,
    ...
    try {
        ...
        const filledFieldsList = (result.filledFields || [])
...
bot.onText(/\/quickfill\s+(\w+)/i, async (msg, match) => {
    ...
    if (!url) {
        await bot.sendMessage(chatId, 
            `❌ Airdrop tidak ditemukan.\n\n` +
            `Tersedia: ${Object.keys(airdrops).join(', ')}`,
            { parse_mode: 'Markdown' }
        );
        return;
    }
        ...
        case 'executeAutofill': {
            ...
            if (formAutoFiller && typeof formAutoFiller.autoSubmitForm === 'function') {
                try {
                    ...
                    const filledList = (result?.filledFields || [])
                        .map(f => `✅ ${f.field}: ${f.value}`)
                        .join('\n') || '❌ No fields filled';
...
${filledList}
                    ...
                    return { 
                        message: `❌ Auto-fill error: ${error.message}\n\n💡 Coba manual atau hubungi admin.` 
                    };
                }
            } else {
                // Fallback manual
